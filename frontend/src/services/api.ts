...
import { SearchResult, SearchResponse, Alert, Cluster, ChangeAnalysis, FeedbackPayload, SystemHealth, TileMetadata } from '../types/api';
...
class ApiService {
  ...
  async searchSemantic(query: string, topK: number = 10): Promise<SearchResponse> {
    if (USE_MOCK) {
      // Simulate network latency for realistic feel
      await new Promise(r => setTimeout(r, 250));
      const results = this.filterMockSearch(query);
      return {
        coverage: 'AVAILABLE',
        requested_location: null,
        message: `Retrieved ${results.length} satellite scenes (mock mode).`,
        available_aois: ['Tumakuru Region', 'Krishna River Basin', 'Ennore Coastal Region'],
        parsed_query: { location: null, change_types: [], years: [] },
        results,
      };
    }
    try {
      ...
      // If backend returns enriched SearchResponse dict with 'coverage' field:
      if (data && typeof data === 'object' && !Array.isArray(data) && 'coverage' in data) {
        return data as SearchResponse;
      }

      // Legacy: backend returns plain array of results
      if (Array.isArray(data)) {
        return {
          coverage: 'AVAILABLE',
          requested_location: null,
          message: `Retrieved ${data.length} satellite scenes.`,
          available_aois: ['Tumakuru Region', 'Krishna River Basin', 'Ennore Coastal Region'],
          parsed_query: { location: null, change_types: [], years: [] },
      ...
      // Fallback to mock on unexpected response structure
      const mockResults = this.filterMockSearch(query);
      return {
        coverage: 'AVAILABLE',
        requested_location: null,
        message: `Retrieved ${mockResults.length} satellite scenes (fallback).`,
        available_aois: ['Tumakuru Region', 'Krishna River Basin', 'Ennore Coastal Region'],
        parsed_query: { location: null, change_types: [], years: [] },
        results: mockResults,
      };
      ...
      return {
        coverage: 'AVAILABLE',
        requested_location: null,
        message: `Retrieved ${mockResults.length} satellite scenes (offline fallback).`,
        available_aois: ['Tumakuru Region', 'Krishna River Basin', 'Ennore Coastal Region'],
        parsed_query: { location: null, change_types: [], years: [] },
        results: mockResults,
      };
